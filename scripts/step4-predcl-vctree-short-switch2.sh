#!/bin/bash
#SBATCH --job-name="VCTree"
#SBATCH --partition=rtx2080ti
#SBATCH --ntasks=4
#SBATCH --gres=gpu:1
#SBATCH --time=3-00:00
#SBATCH --chdir=.
#SBATCH --output=step4vcouts-predcl-switch2.txt
#SBATCH --error=step4vcerrs-predcl-switch2.txt
###SBATCH --test-only
...
python3 train_SG_segmentation_head_rebar.py --eval-only --config-file ../configs/sg_dev_masktransfer_rebar.yaml --num-gpus 1 --resume DATALOADER.NUM_WORKERS 2 \
MODEL.WEIGHTS ../output-step1/model_weights.pth \
OUTPUT_DIR ../output-step3-predcl-vctree-short \
...
MODEL.ROI_BOX_FEATURE_EXTRACTORS.CLASS_LOGITS_WITH_MASK False SOLVER.IMS_PER_BATCH 8 DATASETS.SEG_DATA_DIVISOR 2 \
MODEL.ROI_SCENEGRAPH_HEAD.PREDICTOR 'VCTreeSegmentationPredictorC' MODEL.ROI_HEADS.REFINE_SEG_MASKS False TEST.DETECTIONS_PER_IMAGE 40 \
